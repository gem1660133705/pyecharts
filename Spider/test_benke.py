...
def get_info(page):
    ...
    url='http://college.gaokao.com/schlist/a100/s1/p'+str(page)
    ...
    for sname,location,url,type,membership,website,nature,feature in zip(sname_infos,location_infos,url_infos,type_infos,membership_infos,website_infos,nature_infos,feature_infos):
        data = {
            "sname": sname,
            "location": location,
            "url": url,
        ...
        datas.append(data)
    df = pd.DataFrame(data=datas,columns=['sname', 'location', 'url', 'type', 'membership','website','nature', 'feature'])
    return df



if __name__ == '__main__':
    empty_df = pd.DataFrame(columns=['sname', 'location', 'url', 'type', 'membership', 'website','nature', 'feature'])
    for i in range(1,32):
        df=get_info(i)
        print(df)
